...
export default function App() {
  ...
  useEffect(() => {
    ...
    if (saved) {
      ...
      setShowConsultantReport(typeof data.showConsultantReport === 'boolean' ? data.showConsultantReport : true)
    }

    const dashboard = localStorage.getItem('crftSavedAssessments')
    if (dashboard) {
      setSavedAssessments(JSON.parse(dashboard))
    }
  }, [])
  ...
  useEffect(() => {
    localStorage.setItem('crftSavedAssessments', JSON.stringify(savedAssessments))
  }, [savedAssessments])

  const total = useMemo(() => Object.values(scores).reduce((sum, value) => sum + Number(value), 0), [scores])
  ...
  const oneLineSummary = useMemo(() => getOneLineSummary(scores, total, globalRating), [scores, total, globalRating])
  ...
  const consultantReport = useMemo(
    ...
    [resident, evaluator, rotation, caseName, scores, total, globalRating, priorityRecommendations]
  )
  ...
  const saveCurrentAssessment = () => {
    if (!resident && !caseName && total === 0) {
      alert('Enter at least a resident name, case, or assessment before saving.')
      return
    }
    ...
    setSavedAssessments((prev) => [record, ...prev])
    alert('Assessment saved to dashboard.')
  }

  const loadAssessment = (record) => {
    setResident(record.resident || '')
    setEvaluator(record.evaluator || '')
    setRotation(record.rotation || '')
    setCaseName(record.caseName || '')
    setScores(record.scores || initialScores)
  ...
  return (
    ...
    >
      ...
      {!quickMode && (
        ...
        >
          <h2 style={{ marginTop: 0, fontSize: 20 }}>Multi-Resident Tracking Dashboard</h2>
              ...
              {savedAssessments.map((record) => (
                ...
                >
                  <div style={{ display: 'flex', justifyContent: 'space-between', gap: 10, flexWrap: 'wrap' }}>
                    <div>
                      <div><strong>Resident:</strong> {record.resident || '—'}</div>
                      <div><strong>Case:</strong> {record.caseName || '—'}</div>
                      <div><strong>Rotation:</strong> {record.rotation || '—'}</div>
                      <div><strong>Score:</strong> {record.total}/24 {record.globalRating ? `· ${record.globalRating}` : ''}</div>
                      <div style={{ fontSize: 12, color: '#64748b', marginTop: 4 }}>{record.date}</div>
                    </div>

                    <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', alignItems: 'start' }}>
                      <button
                        onClick={() => loadAssessment(record)}
                        style={{
                          padding: '8px 12px',
                          background: '#0f766e',
                          color: 'white',
                  ...
                  {record.oneLineSummary && (
                    ...
                    >
                      <strong>Summary:</strong> {record.oneLineSummary}
                    </div>
                  )}
                </div>
              ))}
